...
import { writeWorkspaceFile, readWorkspaceFile, workspaceFileExists } from "../../../fs/fileIO.js";
...
describe("File I/O Operations", function () {
  ...
  describe("writeWorkspaceFile", function () {
    ...
    it("should throw an error for paths outside the workspace directory", function () {
      ...
      expect(() => writeWorkspaceFile(workspaceDir, filePath, content)).to.throw(
        "Invalid file path"
      );
    });
  });
  ...
  describe("readWorkspaceFile", function () {
    ...
    it("should throw an error for non-existent files", function () {
      ...
      expect(() => readWorkspaceFile(workspaceDir, filePath)).to.throw("File not found");
    });

    it("should throw an error for paths outside the workspace directory", function () {
      const filePath = "../outside.txt";

      expect(() => readWorkspaceFile(workspaceDir, filePath)).to.throw("Invalid file path");
    });
  });
